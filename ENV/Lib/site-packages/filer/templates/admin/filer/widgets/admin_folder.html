...
{% spaceless %}
    <div class="filer-dropzone filer-dropzone-folder">
        <span class="filerFile">
            ...
            <a href="{{ lookup_url }}" class="related-lookup{% if object %} hidden{% endif %}" id="{{ lookup_name }}" title="{% trans 'Lookup' %}"{% if LTE_DJANGO_1_7 %} onclick="return showRelatedObjectLookupPopup(this);"{% endif %}>
                {% trans "Add Folder" %}塔顶
            </a>
